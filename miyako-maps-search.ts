...
function findSimilarKeywords(keyword: string): string[] {
...
function buildOverpassQuery(keyword: string): string {
...
async function searchOverpassApi(keyword: string): Promise<OverpassPoi[]> {
  try {
...
async function searchFoursquareApi(placeName: string, lat: number, lng: number): Promise<FoursquareVenue | null> {
  try {
...
function buildAddress(tags: OverpassPoi['tags']): string {
  const parts = [];
...
async function enrichWithFoursquare(overpassResults: OverpassPoi[]): Promise<EnrichedPoi[]> {
  ...
  for (const poi of overpassResults) {
    ...
    if (hasApiKey) {
      try {
        const foursquareData = await searchFoursquareApi(
          poi.tags.name || '',
          poi.lat,
          poi.lon
        );
...
function formatPoiForDisplay(poi: EnrichedPoi): string {
  let result = `🏪 ${poi.name}\n`;
  result += `📍 ${poi.address}\n`;
...
async function searchPlacesWithHybridApi(keyword: string): Promise<EnrichedPoi[]> {
  try {
...
function buildMapsSearchUrl(keyword: string): string {
...
async function runInteractiveMode(): Promise<void> {
  ...
  const askQuestion = () => {
    rl.question('\n🔍 検索したいカテゴリの番号またはキーワードを入力してください: ', async (input: string) => {
