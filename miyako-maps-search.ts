...
function searchMaps(keyword: string): void {
  const baseUrl = 'https://www.google.com/maps/search/';
  const mapsUrl = `${baseUrl}${encodeURIComponent(keyword)}`;
  
  console.log(`Opening Google Maps search for: ${keyword}`);
  openUrl(mapsUrl);
...
interface PlaceResult {
  ...
  phone?: string;         // 電話番号
  website?: string;       // サイトURL
  mapsUrl?: string;       // Google Maps店舗情報URL
  types?: string[];
  geometry?: {
    location: {
      lat: number;
...
async function scrapeGoogleMapsResults(searchUrl: string): Promise<PlaceResult[]> {
  let browser;
  try {
  ...
  try {
    ...
    const results = await page.evaluate(() => {
      ...
      for (let i = 0; i < Math.min(linkElements.length, 100); i++) {
        const element = linkElements[i] as HTMLElement;
        const ariaLabel = element.getAttribute('aria-label');
        
        if (!ariaLabel) continue;
        
        // 「·アクセスしたリンク」を除去
        let cleanName = ariaLabel.replace(/·アクセスしたリンク$/, '').trim();
        
        // 明らかに店名ではないものを除外
        if (!cleanName || cleanName.length < 4 || cleanName.length > 80 ||
            cleanName.includes('営業') || cleanName.includes('時間') || 
            cleanName.includes('フィルタ') || cleanName.includes('保存') || 
            cleanName.includes('共有') || cleanName.includes('地図') ||
            cleanName.includes('ログイン') || cleanName.includes('データ') ||
            cleanName.includes('プライバシー') || cleanName.includes('レイヤ') ||
            cleanName.includes('ウェブサイトにアクセス') || cleanName.includes('のウェブサイト') ||
            cleanName.startsWith('「') && cleanName.endsWith('」') ||
            cleanName.match(/^[\d.]+\(/) || cleanName.includes('km')) {
          continue;
        }
        ...
        const place: any = { name: cleanName };
        ...
        if (parentElement) {
          ...
          if (addressElement) {
            ...
            if (addressText && 
                (addressText.includes('平良') || addressText.includes('下地') || addressText.includes('来間') ||
                 addressText.includes('池間') || addressText.includes('多良間') || addressText.includes('伊良部'))) {
    ...
    console.error('❌ Google Mapsからの情報取得に失敗しました:', (error as Error)?.message || error);
    return [];
  } finally {
    if (browser) {
      try {
        ...
        console.error('ブラウザクローズエラー:', (closeError as Error)?.message || closeError);
      }
    }
  }
}

async function searchPlacesInTerminal(keyword: string, bounds: typeof MIYAKOJIMA_BOUNDS): Promise<void> {
...
function buildMapsUrl(keyword?: string): string {
