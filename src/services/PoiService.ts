...
import { OverpassPoi, EnrichedPoi } from "../types.js";
...
export class PoiService {
   ...
   */
  public async enrichWithFoursquare(
    overpassResults: OverpassPoi[],
  ): Promise<EnrichedPoi[]> {
    ...
    for (const poi of overpassResults) {
      const basePoi: EnrichedPoi = {
        ...
        mapsUrl: this.urlService.buildMapsUrl(
          poi.lat,
          poi.lon,
          poi.tags.name || "Unknown",
        ),
      };
